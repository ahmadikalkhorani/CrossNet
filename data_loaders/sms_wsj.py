...
class SmsWsjDataset(Dataset):
    ...
    def __init__(self, 
        ...
        """
        super().__init__()
        assert target in ['early', 'image', 'speech_source', 'direct_path', 'direct'] or target.startswith('direct_path') or target.startswith('RTS'), target
    ...
    def __getitem__(self, index):
        ...
        if 'original_source' not in info['num_samples']:
            # legacy code
            info['num_samples']['original_source'] = info['num_samples']['speech_source']

        target_name = [name.replace('.wav', '_0.wav'), name.replace('.wav', '_1.wav')]
        try:
            if self.target in ['early', 'speech_source', "direct"]:
                ...
                if self.target == 'direct_path':
                    ...
                    for rir_path in info['audio_path']['rir']:
                        ...
                        rir_path = self.sms_wsj_dir  / ('rirs' + rir_path.split('rirs')[-1].replace("/h_", "/ane_h_") )
                        
                        print("*"*20, "   ", self.sms_wsj_dir, "----", rir_path)
                        rir, sr_rir = sf.read(rir_path, dtype='float64')
                        rir_list.append(rir.T)
            ...
            # idx_sorted = determine_assignment_smswsj(info)
            # # sort for location-based training
            # if not (idx_sorted == np.array([0, 1])).all():
            #     target_name.reverse()
            #     target = target[list(idx_sorted)]
            ...
            paras = {
                'index': index,
                'wavname': name,
                'mix_path': "/".join(  str(self.observations[index % len(self.observations)]).split("/")[-3:]  ) ,
                'saveto': target_name,
                'target': self.target,
                'sample_rate': 8000,
                'dataset': 'SMS-WSJ',
