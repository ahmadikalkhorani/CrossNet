...
from models.optim.lr_scheduler import WarmupReduceLROnPlateauScheduler 
...
def on_validation_epoch_end(self: pl.LightningModule, cpu_metric_input: List[Tuple[ndarray, ndarray, int]], N: int = 5) -> None:
    ...
    print("Validation Epoch End - Heavy Metrics")
    ...
    print("Validation Epoch End - Heavy Metrics - Done")
...
def on_test_epoch_end(self: pl.LightningModule, results: List[Dict[str, Any]], cpu_metric_input: List, exp_save_path: str, dataset_name: str = "test"):
    ...
    if self.trainer.is_global_zero:
        ...
        path = os.path.join(folder, f'results_{self.exp_name}_{dataset_name}.json')
        f = open(path, 'w', encoding='utf-8')
        json.dump(results, f, indent=4, cls=MyJsonEncoder)
        f.close()
        
        self.print("results saved to: ", path)  
        
        
        
        # write mean to json
        df = DataFrame(results)
        df.mean(numeric_only=True).to_json(os.path.join(exp_save_path, f'results_mean_{self.exp_name}_{dataset_name}.json'), indent=4)
        df.mean(numeric_only=True).to_json(os.path.join('postprocessing/results', f'results_mean_{self.exp_name}_{dataset_name}.json'), indent=4)
...
def configure_optimizers(
    ...
    print(monitor)
    print(lr_scheduler_kwargs)
    print(lr_scheduler)
    
    if lr_scheduler is not None and len(lr_scheduler) > 0:
        if "init_lr" in lr_scheduler_kwargs.keys():
            optimizer_kwargs["lr"] = lr_scheduler_kwargs["init_lr"]
    
    # optimizer = getattr(torch.optim, optimizer)(self.parameters(), **optimizer_kwargs)
    optimizer = getattr(torch.optim, optimizer)(self.trainer.model.parameters(), **optimizer_kwargs)


    if lr_scheduler is not None and len(lr_scheduler) > 0:
        if lr_scheduler in lr_schedulers_dict.keys(): 
            sch = lr_schedulers_dict[lr_scheduler]
            lr_scheduler = sch["scheduler"]
            reduce_on_plateau = sch["reduce_on_plateau"]
        else:
            lr_scheduler = getattr(torch.optim.lr_scheduler, lr_scheduler)
            reduce_on_plateau = True if isinstance(lr_scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau) else False
        return {
            'optimizer': optimizer,
            'lr_scheduler': {
...
def on_load_checkpoint(
    ...
    # handing mismacthed names or dimensions
